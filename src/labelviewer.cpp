...
void LabelViewer::loadData(std::string filename) {
  labels = belief_tracker::parse_label_json_file(filename);
    updateView();
}
...
void LabelViewer::updateView() {
    ...
    for(auto& current_turn : labels.turns) {
        ...
        low_level_items.push_back(new QTreeWidgetItem(current_item, QStringList(QString::fromStdString(std::string("Semantics: ") + belief_tracker::dialog_acts_to_str(current_turn.semantics)))));
