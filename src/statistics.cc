...
int main(int argc, char* argv[]) {
  ...
  auto statistics_over_dialogs = std::list<std::function<void(belief_tracker::Dialog&)>>({compute_length, compute_dialog_acts_histo, compute_slot_value_histo});
  ...
  for(auto& dpath: dialogs_fullpath) {
    ...
    belief_tracker::Dialog dialog = belief_tracker::parse_dialog_json_file(dialog_filename);
  ...
  std::cout << "You can compile by running two times pdflatex on it" << std::endl;
}
